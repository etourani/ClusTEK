# BO on Aggregation dataset (stageI + StageII)
import os 
import json
import numpy as np
...
DATA = os.path.join(HERE, "..", "..", "data", "synthetic", "aggregation.csv")
OUT  = os.path.join(HERE, "out_aggregation_bo")
...
if __name__ == "__main__":
    ...
    tiny = {"dataset": "Aggregation", "strategy": "bo", "nx": nx, "ny": ny, "h": h, "R": R, "Q": Q}
    with open(os.path.join(OUT, "stageI_row_summary.json"), "w") as f:
        json.dump(tiny, f, indent=2)

    print("\nStage I params are saved to:", OUT)
